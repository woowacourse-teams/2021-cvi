...
===== LIKE_COUNT_ASC 좋아요 오름차순
첫 요청시: boundary 0, id = integer.max

다음 요청시: @boundary 이전 페이징 결과 마지막 글의 좋아요 갯수, @id: 이전 페이징 결과 마지막 글의 id

===== LIKE_COUNT_DESC 좋아요 내림차순
첫 요청시: boundary integer.max, id = integer.max
...
===== COMMENT_COUNT_ASC 댓글 오름차순
첫 요청시: boundary 0, id = integer.max

다음 요청시: @boundary 이전 페이징 결과 마지막 글의 댓글 갯수, @id: 이전 페이징 결과 마지막 글의 id

===== COMMENT_COUNT_DESC 댓글 오름차순
첫 요청시: boundary integer.max, id = integer.max

다음 요청시: @boundary 이전 페이징 결과 마지막 글의 댓글 갯수, @id: 이전 페이징 결과 마지막 글의 id

===== CREATED_AT_DESC 시간순 오름차순
첫 요청시: id = integer.max

다음 요청시: @boundary 아무값이나 상관없음, @id: 이전 페이징 결과 마지막 글의 id
