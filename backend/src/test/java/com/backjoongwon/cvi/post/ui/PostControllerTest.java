...
class PostControllerTest extends ApiDocument {
    ...
    void setUp() {
        ...
        willReturn(user).given(userService).findUserById(any(Long.class));
    }
    ...
    void findByVaccineTypePaging() throws Exception {
        //given
        List<PostResponse> postResponse = new LinkedList<>(Arrays.asList(
                new PostResponse(38L, userResponse, "이건 내용입니다.", 100, 10, 6, true, VaccinationType.PFIZER, LocalDateTime.now()),
                new PostResponse(37L, userResponse, "이건 내용입니다.2", 200, 20, 7, false, VaccinationType.PFIZER, LocalDateTime.now()),
                new PostResponse(36L, userResponse, "이건 내용입니다.3", 300, 30, 19, true, VaccinationType.PFIZER, LocalDateTime.now())
        ));
    ...
    private void 글_타입별_페이징_조회_요청_성공함_게시글없음(ResultActions response, List<PostResponse> postResponse) throws Exception {
        response.andExpect(status().isOk())
                .andExpect(content().json(toJson(postResponse)))
                .andDo(print())
