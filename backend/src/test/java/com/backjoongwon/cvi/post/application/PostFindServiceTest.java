...
public class PostFindServiceTest {
    ...
    /**
     * 골라주기!!!!!!!!!!!!!!!!!!!!!!!!!!!위에처럼 나눠서 할지 이렇게 한방에 끝낼지
     */
    @ParameterizedTest(name = "게시글 정렬 조회 이거하나만 쓸지 각각 쓸지?- 성공")
    @MethodSource
    void findByVaccineTypeSort(VaccinationType vaccinationType, Sort sort, List<String> contentResult) {
        //given
        //when
        List<PostResponse> postResponses = postService.findByVaccineType(vaccinationType, 0, Integer.MAX_VALUE, sort, Integer.MAX_VALUE, optionalUser);
        //then
        assertThat(postResponses).extracting("content").containsExactlyElementsOf(contentResult);
    }

    static Stream<Arguments> findByVaccineTypeSort() {
        return Stream.of(
                Arguments.of(VaccinationType.ALL, Sort.LIKE_COUNT_ASC, Arrays.asList("Test 0", "Test 1", "Test 2")),
                Arguments.of(VaccinationType.ASTRAZENECA, Sort.LIKE_COUNT_ASC, Arrays.asList("Test 0", "Test 1")),
                Arguments.of(VaccinationType.PFIZER, Sort.LIKE_COUNT_ASC, Arrays.asList("Test 2")),
