...
|예시) lastPostId=55&size=10
|불러오고싶은 가장 게시글 ID 55번으로부터 10개
|클라이언트 쪽에서 처음 페이징 요청시 Number.MAX_SAFE_INTEGER 를 실어 보내준다.
|lastPostId와 size를 적지 않고 보낼시 아래의 값으로 요청을 합니다.
|기본값) lastPostId=Long.MAX_VALUE, size=6
|===
==== 정렬
[cols=1*]
|===
...
|===
|All Combined
|/api/v1/posts/paging?lastPostId=55&size=10&sort=CREATED_AT_ASC&fromHoursBefore=24
|(필터) 24시간 내 생성된 게시글
|(정렬) 생성시간 기준 오름차순
|(페이징) 55번 Id를 가진 게시글 중 위의 조건에 부합하는 게시글을 불러온다.
|===
